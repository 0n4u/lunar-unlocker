...
public final class LunarRelogin {
        ...
        if (executor != null) {
            try {
                ...
                if (submit != null) {
                    ...
                    LunarUnlocker.debugLog("LUNAR relogin: submitted to render thread");
                    return;
                }
            }
            catch (Throwable error) {
                LunarUnlocker.debugLog("LUNAR relogin: render submit failed: " + error);
            }
        }

        LunarUnlocker.debugLog("LUNAR relogin: render executor unavailable; running inline");
        replay();
    }
    ...
    private static Object resolveRenderExecutor() {
        try {
            ...
            while (current != null) {
                for (Method method : current.getDeclaredMethods()) {
                    ...
                    if (hasSubmitBridge(candidate)) {
                        LunarUnlocker.debugLog("LUNAR relogin: render executor found structurally via "
                                + method.getName());
                        return candidate;
                    }
                }
            ...
            LunarUnlocker.debugLog("LUNAR relogin: render executor resolution failed: " + error);
        }
        return null;
    }
    ...
    private static void replay() {
        ...
        LunarUnlocker.debugLog("LUNAR relogin: using "
                + (currentRevision ? "current" : "legacy") + " method map ("
                + loginMethods.size() + " enabled services)");
        if (loginMethods.isEmpty()) {
            LunarUnlocker.debugLog("LUNAR relogin: no services enabled; skipping replay");
            return;
        }
        ...
        if (!anyInvoked) {
            LunarUnlocker.debugLog("LUNAR relogin: no login method matched; reconnecting");
    ...
    private static Object resolveManager() {
        try {
            ...
            LunarUnlocker.debugLog("LUNAR relogin: manager resolved "
                    + (manager == null ? "<null>" : manager.getClass().getName()));
            return manager;
        }
        catch (Throwable error) {
            LunarUnlocker.debugLog("LUNAR relogin: manager resolution failed: " + error);
            return null;
        }
    }
    ...
    private static Object resolveServiceHub(Object manager) {
        try {
            ...
            if (looksLikeServiceHub(candidate)) {
                LunarUnlocker.debugLog("LUNAR relogin: service hub resolved by current getter");
                return candidate;
            }
            ...
            while (current != null) {
                for (Method method : current.getDeclaredMethods()) {
                    ...
                    if (looksLikeServiceHub(candidate)) {
                        LunarUnlocker.debugLog("LUNAR relogin: service hub found structurally via "
                                + method.getName());
                        return candidate;
                    }
                }
            ...
            if (legacy != null) {
                ...
                if (looksLikeServiceHub(candidate)) {
                    LunarUnlocker.debugLog("LUNAR relogin: service hub resolved by legacy field");
                    return candidate;
                }
            }
            ...
            while (current != null) {
                for (Field field : current.getDeclaredFields()) {
                    ...
                    if (looksLikeServiceHub(candidate)) {
                        LunarUnlocker.debugLog("LUNAR relogin: service hub found structurally in field "
                                + field.getName());
                        return candidate;
                    }
                }
            ...
            LunarUnlocker.debugLog("LUNAR relogin: service hub resolution failed: " + error);
            return null;
        }

        LunarUnlocker.debugLog("LUNAR relogin: asset websocket service hub not found");
        return null;
    }
    ...
    private static boolean invokeLogin(Object serviceHub, String methodName) {
        try {
            ...
            if (method == null || method.getParameterTypes().length != 0) {
                LunarUnlocker.debugLog("LUNAR relogin: method " + methodName
                        + " not found on " + serviceHub.getClass().getName());
                return false;
            }
            method.setAccessible(true);
            method.invoke(serviceHub);
            LunarUnlocker.debugLog("LUNAR relogin: invoked " + methodName);
            return true;
        }
        catch (Throwable error) {
            LunarUnlocker.debugLog("LUNAR relogin: invoking " + methodName
                    + " failed: " + error);
            return false;
        }
    }
    ...
    private static void fallbackReconnect(Object serviceHub) {
        try {
            ...
            if (reconnect == null) {
                LunarUnlocker.debugLog("LUNAR relogin: reconnect() unavailable; "
                        + "login will fire on the next websocket reconnect");
                return;
            }
            reconnect.setAccessible(true);
            reconnect.invoke(serviceHub);
            LunarUnlocker.debugLog("LUNAR relogin: reconnect() invoked");
        }
        catch (Throwable error) {
            LunarUnlocker.debugLog("LUNAR relogin: reconnect() failed: " + error);
        }
    }
