...
public final class PrometheusHooks {
    ...
    private static File storageDirectory() {
        String nativeDirectory = System.getProperty("lunarunlocker.directory");
        File base;
        if (nativeDirectory != null && !nativeDirectory.trim().isEmpty()) {
            base = new File(nativeDirectory, ".lunarunlocker");
        } else {
            String appData = System.getenv("APPDATA");
            base = new File(appData == null ? System.getProperty("user.home")
                    : appData, ".lunarunlocker");
        }
        File directory = new File(base, "lunar");
        if (!directory.exists()) {
            directory.mkdirs();
        }
        LunarUnlocker.debugLog("LUNAR storage: lunarunlocker.directory='"
    ...
    private static void runCallback(Object rpcCallback, Object response) {
        if (rpcCallback == null) {
            LunarUnlocker.debugLog("LUNAR callback: null callback, skipping");
            return;
        }
        try {
            ...
            LunarUnlocker.debugLog("LUNAR callback: delivered");
        }
        catch (Throwable error) {
            LunarUnlocker.debugLog("Lunar hook: callback failed: " + error);
            ...
            Throwable cause = error instanceof java.lang.reflect.InvocationTargetException
                    ...
                    : error;
            if (cause != null) {
                LunarUnlocker.debugLog("Lunar hook: callback cause: " + cause);
                for (StackTraceElement element : cause.getStackTrace()) {
                    LunarUnlocker.debugLog("  at " + element);
                }
            }
        }
    }
    ...
    private static Object buildSingle(Object messageClass, String[] setters,
        ...
        try {
            ...
            LunarUnlocker.debugLog("Lunar hook: build failed: " + error);
            return null;
        }
    }
    ...
    private static Object newBuilder(Class<?> messageClass) {
        try {
            ...
            LunarUnlocker.debugLog("Lunar hook: newBuilder failed: " + error);
            return null;
        }
    }
    ...
    private static Object invokeBuilder(Object builder, String setter,
        ...
        try {
            ...
            LunarUnlocker.debugLog("Lunar hook: " + setter + " failed: " + error);
            return builder;
        }
    }
    ...
    public static void badgeLogin(Object rpcCallback) {
        try {
            LunarUnlocker.debugLog("LUNAR hook badgeLogin invoked");
            ...
            LunarUnlocker.debugLog("LUNAR hook badgeLogin delivered");
            runCallback(rpcCallback, response);
        }
        catch (Throwable error) {
            LunarUnlocker.debugLog("Lunar hook: badgeLogin failed: " + error);
        }
    }

    public static void badgeEquip(Object equipBadgeRequest) {
        try {
            LunarUnlocker.debugLog("LUNAR hook badgeEquip invoked");
            ...
            LunarUnlocker.debugLog("Lunar hook: badgeEquip failed: " + error);
        }
    }
    ...
    public static void cosmeticLogin(Object rpcCallback) {
        try {
            LunarUnlocker.debugLog("LUNAR hook cosmeticLogin invoked");
            ...
            
            {
                ...
                if (ids.isEmpty()) {
                    LunarUnlocker.debugLog("Lunar cosmetic catalog unavailable; no ownership applied");
                } else {
                    ...
                    try {
                        ...
                        LunarUnlocker.debugLog("LUNAR hook cosmeticLogin ownedCosmetics set ("
                                + owned.size() + ")");
                    }
                    catch (Throwable error) {
                        LunarUnlocker.debugLog("Lunar hook: ownedCosmetics failed: " + error);
                    }
                }
            }
            ...
            if (LunarUnlockSettings.current().lunarPlusAppearanceEnabled()) {
                try {
                    ...
                    LunarUnlocker.debugLog("LUNAR hook cosmeticLogin plusIcon fields set");
                }
                catch (Throwable error) {
                    LunarUnlocker.debugLog("Lunar hook: plusIcon fields failed: " + error);
                }
            }
            Object response = builder.getClass().getMethod("build").invoke(builder);
            LunarUnlocker.debugLog("LUNAR hook cosmeticLogin delivered");
            runCallback(rpcCallback, response);
        }
        catch (Throwable error) {
            LunarUnlocker.debugLog("Lunar hook: cosmeticLogin failed: " + error);
        }
    }

    public static void cosmeticUpdateOutfit(Object updateOutfitRequest) {
        try {
            LunarUnlocker.debugLog("LUNAR hook cosmeticUpdateOutfit invoked");
            Method getOutfit = updateOutfitRequest.getClass().getMethod("getOutfit");
            Object outfit = getOutfit.invoke(updateOutfitRequest);
            writeOutfit(outfit);
        }
        catch (Throwable error) {
            LunarUnlocker.debugLog("Lunar hook: cosmeticUpdateOutfit failed: " + error);
        }
    }
    ...
    public static void emoteLogin(Object rpcCallback) {
        try {
            LunarUnlocker.debugLog("LUNAR hook emoteLogin invoked");
            ...
            Object response = builder.getClass().getMethod("build").invoke(builder);
            LunarUnlocker.debugLog("LUNAR hook emoteLogin delivered");
            runCallback(rpcCallback, response);
        }
        catch (Throwable error) {
            LunarUnlocker.debugLog("Lunar hook: emoteLogin failed: " + error);
        }
    }

    public static void emoteUseEmote(Object rpcCallback) {
        try {
            LunarUnlocker.debugLog("LUNAR hook emoteUseEmote invoked");
            ...
            Object response = builder.getClass().getMethod("build").invoke(builder);
            LunarUnlocker.debugLog("LUNAR hook emoteUseEmote delivered");
            runCallback(rpcCallback, response);
        }
        catch (Throwable error) {
            LunarUnlocker.debugLog("Lunar hook: emoteUseEmote failed: " + error);
        }
    }

    public static void emoteUpdateEquippedEmotes(Object updateRequest) {
        try {
            LunarUnlocker.debugLog("LUNAR hook emoteUpdateEquippedEmotes invoked");
            ...
            LunarUnlocker.debugLog("Lunar hook: emoteUpdateEquippedEmotes failed: " + error);
        }
    }
    ...
    public static void sprayLogin(Object rpcCallback) {
        try {
            LunarUnlocker.debugLog("LUNAR hook sprayLogin invoked");
            ...
            Object response = builder.getClass().getMethod("build").invoke(builder);
            LunarUnlocker.debugLog("LUNAR hook sprayLogin delivered");
            runCallback(rpcCallback, response);
        }
        catch (Throwable error) {
            LunarUnlocker.debugLog("Lunar hook: sprayLogin failed: " + error);
        }
    }

    public static void sprayUseSpray(Object rpcCallback) {
        try {
            LunarUnlocker.debugLog("LUNAR hook sprayUseSpray invoked");
            ...
            Object response = builder.getClass().getMethod("build").invoke(builder);
            LunarUnlocker.debugLog("LUNAR hook sprayUseSpray delivered");
            runCallback(rpcCallback, response);
        }
        catch (Throwable error) {
            LunarUnlocker.debugLog("Lunar hook: sprayUseSpray failed: " + error);
        }
    }

    public static void sprayUpdateEquippedSprays(Object updateRequest) {
        try {
            LunarUnlocker.debugLog("LUNAR hook sprayUpdateEquippedSprays invoked");
            Method getList = updateRequest.getClass().getMethod("getEquippedSpraysList");
            Object list = getList.invoke(updateRequest);
            writeDelimited("sprays.bin", list);
        }
        catch (Throwable error) {
            LunarUnlocker.debugLog("Lunar hook: sprayUpdateEquippedSprays failed: " + error);
        }
    }
    ...
    public static void jamLogin(Object rpcCallback) {
        try {
            LunarUnlocker.debugLog("LUNAR hook jamLogin invoked");
            ...
            Object response = builder.getClass().getMethod("build").invoke(builder);
            LunarUnlocker.debugLog("LUNAR hook jamLogin delivered (" + jams.size() + " jams)");
            runCallback(rpcCallback, response);
        }
        catch (Throwable error) {
            LunarUnlocker.debugLog("Lunar hook: jamLogin failed: " + error);
        }
    }
    ...
    private static void writeBadge(int badgeId) {
        ...
        try (DataOutputStream os = new DataOutputStream(
            ...
            os.writeInt(badgeId);
            os.flush();
            LunarUnlocker.debugLog("LUNAR storage: writeBadge OK -> "
                    + file.getAbsolutePath() + " id=" + badgeId
                    + " (" + file.length() + " bytes)");
        }
        catch (Throwable error) {
            LunarUnlocker.debugLog("Lunar hook: writeBadge failed: " + error
    ...
    private static int readBadge() {
        ...
        try (DataInputStream is = new DataInputStream(
            ...
            if (file.length() >= 4L) {
                ...
                LunarUnlocker.debugLog("LUNAR storage: readBadge OK from "
                        + file.getAbsolutePath() + " id=" + badgeId);
                return badgeId;
            }
            ...
            LunarUnlocker.debugLog("LUNAR storage: readBadge none at "
    ...
    private static OutfitFile readOutfit() {
        ...
        try (InputStream is = new FileInputStream(file)) {
            ...
            LunarUnlocker.debugLog("LUNAR storage: readOutfit OK from "
                    + file.getAbsolutePath() + " (" + file.length() + " bytes)");
            return new OutfitFile(outfit);
        }
        catch (Throwable error) {
            LunarUnlocker.debugLog("LUNAR storage: readOutfit none at "
                    + file.getAbsolutePath() + " (" + error + ")");
            return null;
        }
    }
    ...
    private static void writeOutfit(Object outfit) {
        ...
        try (OutputStream os = new FileOutputStream(file)) {
            outfit.getClass().getMethod("writeTo", OutputStream.class)
                    .invoke(outfit, os);
            os.flush();
            LunarUnlocker.debugLog("LUNAR storage: writeOutfit OK -> "
                    + file.getAbsolutePath() + " (" + file.length() + " bytes)");
        }
        catch (Throwable error) {
            LunarUnlocker.debugLog("Lunar hook: writeOutfit failed: " + error
                    + " path=" + file.getAbsolutePath());
        }
    }
    ...
    private static void writeDelimited(String fileName, Object list) {
        ...
        try (OutputStream os = new FileOutputStream(storageFile(fileName))) {
            ...
            LunarUnlocker.debugLog("Lunar hook: writeDelimited failed: " + error);
        }
    }
}
