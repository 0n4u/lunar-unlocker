package gg.lunarunlocker.config;
...
public final class LocalConfigStore {
    ...
    public static File baseDirectory() {
        ...
        if (nativeDirectory != null && !nativeDirectory.trim().isEmpty()) {
            return new File(nativeDirectory, ".lunarunlocker");
        }
        String appData = System.getenv("APPDATA");
        if (appData == null || appData.trim().isEmpty()) {
            appData = System.getProperty("user.home");
        }
        return new File(appData, ".lunarunlocker");
    }
    ...
    private static void migrateLegacyConfig(File targetDirectory) {
        ...
        File legacy = new File(new File(appData, ".lunarunlocker"), FILE_NAME);
        if (!legacy.isFile()) {
            return;
        }
        try {
