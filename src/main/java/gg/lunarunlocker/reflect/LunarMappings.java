...
public final class LunarMappings {
    ...
    public static boolean isRuntimePresent() {
        ...
        LunarUnlocker.debugLog("LUNAR detection: " + (found ? "YES" : "NO")
    ...
    private static boolean hasLunarMarker(Set<ClassLoader> loaders) {
        for (ClassLoader loader : loaders) {
            for (String marker : MARKER_RESOURCES) {
                try {
                    ...
                    if (resource != null) {
                        LunarUnlocker.debugLog("LUNAR marker found: " + marker
                                + " @ " + resource + " [" + loader + "]");
                        return true;
                    }
                }
        ...
        for (ClassLoader loader : loaders) {
            try {
                ...
                LunarUnlocker.debugLog("LUNAR marker found via Class.forName: "
                        + probeName + " [" + loader + "]");
                return true;
            }
            catch (ClassNotFoundException ignored) {
