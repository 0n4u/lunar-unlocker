...
import gg.lunarunlocker.reflect.Badlion189Mappings;
import gg.lunarunlocker.reflect.Fabric12111Mappings;
import gg.lunarunlocker.reflect.Fabric262Mappings;
import gg.lunarunlocker.reflect.NeoForge1201Mappings;
import gg.lunarunlocker.reflect.NeoForge1211Mappings;
import gg.lunarunlocker.reflect.Type;
import gg.lunarunlocker.reflect.Vanilla1122Mappings;
import gg.lunarunlocker.reflect.Vanilla1211Mappings;
import gg.lunarunlocker.reflect.Vanilla1201Mappings;
import gg.lunarunlocker.reflect.Vanilla1206Mappings;
import gg.lunarunlocker.reflect.Vanilla12111Mappings;
import gg.lunarunlocker.reflect.Vanilla1710Mappings;
import gg.lunarunlocker.reflect.Vanilla189Mappings;
import gg.lunarunlocker.reflect.Vanilla262Mappings;
import gg.lunarunlocker.ui.click.GuiScreenNativeCallbackBridge;
import gg.lunarunlocker.utils.Base64Util;
...
public class NativeBridge {
    ...
    public static void start() throws Throwable {
        ...
        LunarUnlocker unlocker = new LunarUnlocker();
        NativeBridge.invokeVoidInit(unlocker, "loadMappings");
        ...
        if (!unlocker.initAccountInfo()) {
            NativeBridge.sce("WARN initAccountInfo; continuing without account information");
        } else {
            NativeBridge.sce("OK initAccountInfo");
        }
        NativeBridge.invokeVoidInit(unlocker, "initializeManagers");
    }

    private static void invokeVoidInit(LunarUnlocker unlocker, String name) {
        for (Method method : LunarUnlocker.class.getDeclaredMethods()) {
            if (!method.getName().equals(name) || !method.getReturnType().equals(Void.TYPE)) continue;
            NativeBridge.sce("LOAD " + name);
            method.setAccessible(true);
            try {
                method.invoke(unlocker, new Object[0]);
    ...
    public static Class<?> gvc(String internalName) {
        ...
        if (mappingVersion == 110 || mappingVersion == 100) {
            if (fabric262Runtime) {
