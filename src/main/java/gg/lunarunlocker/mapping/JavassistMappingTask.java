...
import gg.lunarunlocker.asm.transform.ClassTransformer;
import gg.lunarunlocker.event.EventBus;
import gg.lunarunlocker.event.IEvent;
import gg.lunarunlocker.mapping.expr.MethodCallInjectionExprEditor;
import gg.lunarunlocker.mapping.expr.MethodCallLineCaptureExprEditor;
import gg.lunarunlocker.mapping.expr.MethodCallReplacementExprEditor;
...
implements MappingTask {
    ...
    public String z(Class<?> clazz, String string, String string2, String string3, String string4) {
        ...
        if (n != 0) {
            ...
            if (bl4) {
                ...
                Method method = (Method)EventBus.findEventListenersAccessor(eventClass);
    ...
    private void dumpFailedCommit(int errorCode) {
        try {
            File directory = new File(gg.lunarunlocker.config.LocalConfigStore.baseDirectory(), "LunarUnlockerRecovery");
            if (!directory.exists()) {
                directory.mkdirs();
            }
            ...
            LunarUnlocker.debugLog("Failed mapping commit bytecode dumped to " + directory.getAbsolutePath());
        }
        catch (Exception exception) {
            LunarUnlocker.logThrowable(exception);
        }
    ...
    public CtBehavior F(MappingMethod mappingMethod) {
        try {
            ...
            LunarUnlocker.logThrowable(notFoundException);
            return null;
        }
    }
    ...
    public CtClass x() {
        try {
            ...
            LunarUnlocker.logThrowable(iOException);
            return null;
        }
    }
    ...
    public void l(MappingMethod mappingMethod) {
        ...
        try {
            ...
            LunarUnlocker.logThrowable(cannotCompileException);
        }
    }
