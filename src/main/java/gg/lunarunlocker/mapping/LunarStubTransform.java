...
import gg.lunarunlocker.lunar.PrometheusHooks;
...
public final class LunarStubTransform extends ClassTransformer {
    private static final String HOOK_OWNER = "gg/lunarunlocker/lunar/PrometheusHooks";
    ...
    public LunarStubTransform(String targetClassName,
        ...
        if (this.inactive) {
            LunarUnlocker.debugLog("LUNAR transform inactive (class unavailable): "
                    + targetClassName);
        } else {
            LunarUnlocker.debugLog("LUNAR transform registered: " + targetClassName);
        }
    }
    ...
    public void transform() {
        ...
        for (int i = 0; i < methodNames.length; ++i) {
            ...
            if (method == null) {
                LunarUnlocker.debugLog("LUNAR method not present in this revision: "
            ...
            LunarUnlocker.debugLog("LUNAR method rewritten: "
                    + this.targetClassName + "#" + methodNames[i]
                    + " -> " + hookNames[i]);
        }
    }
    ...
    public int commit() {
        ...
        LunarUnlocker.debugLog("LUNAR commit " + this.targetClassName
                + (result == 0 ? " OK" : " -> " + result));
        return result;
    }
