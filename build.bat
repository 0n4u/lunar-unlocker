...
title LunarUnlocker v4.21 - One-Click Build
cd /d "%~dp0"
...
echo.
echo ============================================================
echo   LunarUnlocker v4.21 - one-click build
echo ============================================================
echo.
...
if "%JAVA_ONLY%"=="1" (
    ...
    if not exist "build\libs\lunarunlocker-product-recovery-4.21-recovered-injection.jar" goto :fail_build
    ...
    if errorlevel 1 goto :fail_build
    if not exist "build\injection\LunarUnlocker.exe" goto :fail_build
)

mkdir "%BUILT_DIR%" 2>nul
if exist "build\injection\LunarUnlocker.exe" (
    copy /y "build\injection\LunarUnlocker.exe" "%BUILT_DIR%" >nul
    copy /y "build\injection\LunarUnlockerNative.dll" "%BUILT_DIR%" >nul
    echo.
    echo ============================================================
    echo   BUILD COMPLETE
