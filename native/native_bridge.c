...
static void log_jvmti_failure(const wchar_t *operation, jvmtiError error,
    ...
    lunarunlocker_log(L"%ls failed: jvmti=%d (%hs), target=%hs", operation,
...
static int contains_lunarunlocker_callback(const unsigned char *bytes, jint length) {
    return contains_bytes(bytes, length, "gg/lunarunlocker/")
            || contains_bytes(bytes, length, "gg.lunarunlocker.");
}
...
static void update_persisted_class(JNIEnv *env, jclass target,
    ...
    unsigned char *copy = NULL;
    int should_persist = contains_lunarunlocker_callback(bytes, length);

    if (should_persist) {
        copy = (unsigned char *)HeapAlloc(
                GetProcessHeap(), 0, (SIZE_T)length);
        if (copy == NULL) {
            lunarunlocker_log(L"unable to retain transformed class bytecode");
            return;
        }
        memcpy(copy, bytes, (size_t)length);
    }
    ...
    if (entry == NULL || entry->target == NULL) {
        ...
        ReleaseSRWLockExclusive(&g_persisted_class_lock);
        lunarunlocker_log(L"unable to retain transformed class identity");
        return;
    }
    entry->bytes = copy;
    entry->length = length;
...
jint lunarunlocker_initialize_jvmti(JavaVM *vm) {
    ...
    if (get_env_result != JNI_OK || g_jvmti == NULL) {
        lunarunlocker_log(L"JVMTI 1.2 is unavailable: %d", get_env_result);
        g_jvmti = NULL;
        return JNI_ERR;
    }
    ...
    if (error != JVMTI_ERROR_NONE) {
        lunarunlocker_log(L"GetPotentialCapabilities failed: %d", error);
        return JNI_ERR;
    }
    ...
    if (error != JVMTI_ERROR_NONE) {
        lunarunlocker_log(L"AddCapabilities failed: %d", error);
        return JNI_ERR;
    }
    ...
    if (error != JVMTI_ERROR_NONE) {
        lunarunlocker_log(L"SetEventCallbacks failed: %d", error);
        return JNI_ERR;
    }
    error = (*g_jvmti)->SetEventNotificationMode(g_jvmti, JVMTI_ENABLE,
            JVMTI_EVENT_CLASS_FILE_LOAD_HOOK, NULL);
    if (error != JVMTI_ERROR_NONE) {
        lunarunlocker_log(L"Enable ClassFileLoadHook failed: %d", error);
        return JNI_ERR;
    }
...
static void JNICALL native_cpy(
    ...
    if (memory == NULL) {
        lunarunlocker_log(L"cpy GlobalAlloc failed: %lu", GetLastError());
        goto cleanup;
    }
    destination = GlobalLock(memory);
    if (destination == NULL) {
        lunarunlocker_log(L"cpy GlobalLock failed: %lu", GetLastError());
        goto cleanup;
    }
    ...
    if (!clipboard_open) {
        lunarunlocker_log(L"cpy OpenClipboard failed: %lu", GetLastError());
        goto cleanup;
    }
    if (!EmptyClipboard()) {
        lunarunlocker_log(L"cpy EmptyClipboard failed: %lu", GetLastError());
        goto cleanup;
    }
    if (SetClipboardData(CF_TEXT, memory) == NULL) {
        lunarunlocker_log(L"cpy SetClipboardData failed: %lu", GetLastError());
        goto cleanup;
    }
    memory = NULL;
...
static jbyteArray JNICALL native_gcb(JNIEnv *env, jclass bridge, jclass target) {
    ...
    if (error == JVMTI_ERROR_NONE && g_capture_bytes != NULL
        ...
        lunarunlocker_log(L"gcb captured byte count: %d", g_capture_length);
    }
...
static LRESULT CALLBACK lwjgl3_window_proc(
    ...
    if (env != NULL && g_bridge_class != NULL && g_bridge_om != NULL
        ...
        if ((*env)->ExceptionCheck(env)) {
            lunarunlocker_log_pending_exception(env, L"LWJGL3 window input callback");
            handled = JNI_FALSE;
        }
    }
    if (attached) {
...
static void register_lwjgl3_window(JNIEnv *env) {
    ...
    if (window == NULL) {
        lunarunlocker_log(L"trs step 23: LWJGL3 window was not found");
        return;
    }
    ...
    if (original == NULL && GetLastError() != ERROR_SUCCESS) {
        lunarunlocker_log(L"failed to subclass LWJGL3 window: %lu", GetLastError());
        return;
    }
    ...
    if (g_bridge_class != NULL && g_bridge_wh != NULL) {
        ...
        if ((*env)->ExceptionCheck(env)) {
            lunarunlocker_log_pending_exception(env, L"initialize LWJGL3 window handle");
        }
    }
    lunarunlocker_log(L"subclassed LWJGL3 window for input notifications");
}
...
static void JNICALL native_trs(JNIEnv *env, jclass bridge, jint step) {
    ...
    (void)bridge;
    lunarunlocker_loader_report_progress((int)step);
    ...
    if (error != JVMTI_ERROR_NONE || classes == NULL) {
        lunarunlocker_log(L"trs GetLoadedClasses failed: %d", error);
        return;
    }
    for (index = 0; index < class_count; ++index) {
        char *signature = NULL;
    ...
    if (windows_display != NULL) {
        ...
        if ((*env)->RegisterNatives(env, windows_display, &update_method, 1) == JNI_OK) {
            ...
            lunarunlocker_log(L"registered org.lwjgl.opengl.WindowsDisplay.nUpdate");
        } else {
            lunarunlocker_log_pending_exception(env, L"RegisterNatives WindowsDisplay.nUpdate");
        }
    } else {
        lunarunlocker_log(L"trs step 23: WindowsDisplay is not loaded; trying LWJGL3 window");
...
static void JNICALL native_sce(JNIEnv *env, jclass bridge, jstring message) {
    ...
    (void)bridge;
    if (message == NULL) {
        lunarunlocker_log(L"client error report: <null>");
        return;
    }
    chars = (*env)->GetStringUTFChars(env, message, NULL);
    if (chars != NULL) {
        lunarunlocker_log(L"client error report: %hs", chars);
...
static jstring JNICALL bridge_gat(JNIEnv *env, jclass bridge) {
    ...
    if (!lunarunlocker_loader_bootstrap_initialize()
            || lunarunlocker_loader_bootstrap_failed()) {
    ...
    token = lunarunlocker_loader_access_token();
...
jint lunarunlocker_register_native_bridge(JNIEnv *env, jclass bridge_class) {
    ...
    if (result != JNI_OK) {
        lunarunlocker_log_pending_exception(env, L"RegisterNatives NativeBridge");
        return result;
    }
    ...
    if (g_bridge_class == NULL || g_bridge_om == NULL || g_bridge_wh == NULL) {
        lunarunlocker_log_pending_exception(env, L"resolve NativeBridge input callbacks");
        return JNI_ERR;
    }
    lunarunlocker_log(L"registered NativeBridge methods (including dch)");
    return JNI_OK;
}
