...
enum {
    LUNARUNLOCKER_TOKEN_UNINITIALIZED = 0,
    LUNARUNLOCKER_TOKEN_STANDALONE = 1,
    LUNARUNLOCKER_TOKEN_ONLINE = 2,
    LUNARUNLOCKER_TOKEN_FAILED = 3
};

static INIT_ONCE g_bootstrap_once = INIT_ONCE_STATIC_INIT;
static SRWLOCK g_controller_lock = SRWLOCK_INIT;
static volatile LONG g_token_state = LUNARUNLOCKER_TOKEN_UNINITIALIZED;
...
static void object_name(wchar_t *output, size_t capacity,
    ...
    _snwprintf_s(output, capacity, _TRUNCATE,
            L"Local\\LunarUnlocker.%ls.%lu", kind, process_id);
}
...
static BOOL CALLBACK initialize_bootstrap(
    ...
    wchar_t name[96];
    HANDLE mapping;
    LunarUnlockerBootstrapV2 *block;
    LONG state = LUNARUNLOCKER_TOKEN_FAILED;
    ...
    if (mapping == NULL) {
        if (GetLastError() == ERROR_FILE_NOT_FOUND) {
            ...
            state = LUNARUNLOCKER_TOKEN_STANDALONE;
        }
        InterlockedExchange(&g_token_state, state);
        return TRUE;
    }

    block = (LunarUnlockerBootstrapV2 *)MapViewOfFile(mapping,
            FILE_MAP_READ | FILE_MAP_WRITE, 0, 0, sizeof(*block));
    if (block != NULL) {
        if (block->magic == LUNARUNLOCKER_BOOTSTRAP_MAGIC
            ...
            if (SetEnvironmentVariableA("LUNARUNLOCKER_ONLINE_BASE_URL",
                        block->service_http_base)
                    && SetEnvironmentVariableA("LUNARUNLOCKER_ZEUS_ADDRESS",
                        zeus_address)
                    && connect_controller(block->controller_port)
                    && request_access_token()) {
                state = LUNARUNLOCKER_TOKEN_ONLINE;
                InterlockedExchange((volatile LONG *)&block->status,
                        LUNARUNLOCKER_BOOTSTRAP_STATUS_CONSUMED);
            } else {
                close_controller_locked();
                SecureZeroMemory(g_access_token, sizeof(g_access_token));
                InterlockedExchange((volatile LONG *)&block->status,
                        LUNARUNLOCKER_BOOTSTRAP_STATUS_FAILED);
            }
        } else {
            InterlockedExchange((volatile LONG *)&block->status,
                    LUNARUNLOCKER_BOOTSTRAP_STATUS_FAILED);
...
void lunarunlocker_loader_bootstrap_clear(void) {
    ...
    close_controller_locked();
    SecureZeroMemory(g_access_token, sizeof(g_access_token));
    InterlockedExchange(&g_token_state, LUNARUNLOCKER_TOKEN_UNINITIALIZED);
    ReleaseSRWLockExclusive(&g_controller_lock);
}
