...
static int materialize_embedded_dll(
    ...
    if (GetModuleFileNameW(NULL, exe_path, MAX_PATH) != 0
        ...
        _snwprintf_s(output, capacity, _TRUNCATE,
                L"%ls\\LunarUnlockerNative.dll", exe_path);
    ...
    _snwprintf_s(directory,
            ...
            L"%ls\\LunarUnlockerRecovery", client_root);
...
static void render_selector(const process_candidate *candidates, size_t count,
    ...
    wprintf(L"LunarUnlocker Injector\n");
    wprintf(L"DLL: %ls\n\n", dll_path);
...
static int inject_library(DWORD process_id, const wchar_t *dll_path) {
    ...
    if (result == 0) {
        fwprintf(stderr, L"LoadLibraryW returned, but the DLL was not mapped. "
                        L"Check the bootstrap failure info in lunarunlocker-native.log.\n");
        goto cleanup;
    }
...
static void usage(const wchar_t *program) {
    fwprintf(stderr,
            ...
            L"This program always uses the embedded LunarUnlockerNative.dll and never loads an external DLL.\n"
            L"The embedded DLL is extracted to <exe>\\.lunarunlocker\\LunarUnlockerRecovery before injection.\n",
            program, program);
}
...
int console_main(int argc, wchar_t **argv) {
    ...
    if (!materialize_embedded_dll((DWORD)process_id, dll_path, MAX_PATH)) {
        fwprintf(stderr, L"Failed to extract the embedded LunarUnlockerNative.dll.\n");
        return 2;
    }
    if (argc != 1) {
