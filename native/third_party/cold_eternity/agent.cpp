...
void writeStatus(const std::string& status) {
    if (status == "SUCCESS") {
        ...
        lunarunlocker_loader_report_failure(status.c_str());
...
void initializeSelectionPath() {
    ...
    const std::filesystem::path stateRoot =
        recoveryDirectory.filename() == L"LunarUnlockerRecovery"
            ? recoveryDirectory.parent_path()
            : recoveryDirectory / L".lunarunlocker";
...
void runAgent() {
    ...
    const std::wstring mutexName = L"Local\\LunarUnlocker_DirectEngine_" +
