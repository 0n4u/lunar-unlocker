#include "lunarunlocker_native.h"
...
#define LUNARUNLOCKER_PRODUCT_JAR_RESOURCE_ID 421
...
static int injector_directory(wchar_t *output, size_t capacity) {
    ...
    for (marker = wcsstr(output, L"\\.lunarunlocker\\");
            marker != NULL;
            marker = wcsstr(marker + 1, L"\\.lunarunlocker\\")) {
        last = marker;
    }
...
static int client_directory(wchar_t *output, size_t capacity) {
    ...
    base = wcslen(output);
    if (base + wcslen(L"\\.lunarunlocker") + 1 > capacity) {
        return 0;
    }
    wcscat_s(output, capacity, L"\\.lunarunlocker");
...
static void ensure_log_path(void) {
    ...
    _snwprintf_s(g_log_file, MAX_PATH, _TRUNCATE,
            L"%ls\\lunarunlocker-native-%lu-%04u%02u%02u-%02u%02u%02u.log",
...
static void set_lunarunlocker_directory_property(JNIEnv *env) {
    wchar_t directory[MAX_PATH];
    ...
    key = (*env)->NewStringUTF(env, "lunarunlocker.directory");
...
void lunarunlocker_log_pending_exception(JNIEnv *env, const wchar_t *context) {
    ...
    if (env == NULL || !(*env)->ExceptionCheck(env)) {
        lunarunlocker_log(L"%ls failed without a Java exception", context);
        return;
    }
    ...
    if (text == NULL || (*env)->ExceptionCheck(env)) {
        (*env)->ExceptionClear(env);
        lunarunlocker_log(L"%ls raised an unreadable Java exception", context);
        return;
    }
    ...
    if (characters != NULL) {
        ...
        lunarunlocker_log(L"%ls: %ls", context, buffer);
    }
}
...
static int materialize_embedded_product_jar(
    ...
    resource = FindResourceW(g_module,
            MAKEINTRESOURCEW(LUNARUNLOCKER_PRODUCT_JAR_RESOURCE_ID),
            MAKEINTRESOURCEW(10));
    if (resource == NULL) {
        lunarunlocker_log(L"embedded product JAR resource %d is missing",
                LUNARUNLOCKER_PRODUCT_JAR_RESOURCE_ID);
        return 0;
    }
    ...
    if (bytes == NULL || size < 4 || bytes[0] != 'P' || bytes[1] != 'K') {
        lunarunlocker_log(L"embedded product JAR resource is invalid");
        return 0;
    }
    if (!client_directory(temp_root,
            sizeof(temp_root) / sizeof(temp_root[0]))) {
        lunarunlocker_log(L"client_directory failed for embedded product JAR");
        return 0;
    }
    _snwprintf_s(temp_directory,
            sizeof(temp_directory) / sizeof(temp_directory[0]), _TRUNCATE,
            L"%ls\\LunarUnlockerRecovery", temp_root);
    if (!CreateDirectoryW(temp_directory, NULL)
            && GetLastError() != ERROR_ALREADY_EXISTS) {
        lunarunlocker_log(L"CreateDirectoryW failed: %lu", GetLastError());
        return 0;
    }
    if (_snwprintf_s(jar_path, jar_capacity, _TRUNCATE,
            L"%ls\\LunarUnlocker-product-%lu.jar", temp_directory,
            GetCurrentProcessId()) < 0) {
        lunarunlocker_log(L"temporary product JAR path is too long");
        return 0;
    }
    ...
    if (file == INVALID_HANDLE_VALUE) {
        lunarunlocker_log(L"CreateFileW for embedded product JAR failed: %lu",
                GetLastError());
        return 0;
    }
    while (offset < size) {
        ...
        if (!WriteFile(file, bytes + offset, remaining, &written, NULL)
                || written == 0) {
            lunarunlocker_log(L"WriteFile for embedded product JAR failed: %lu",
                    GetLastError());
            goto cleanup;
        }
        offset += written;
    ...
    if (!FlushFileBuffers(file)) {
        lunarunlocker_log(L"FlushFileBuffers for embedded product JAR failed: %lu",
                GetLastError());
        goto cleanup;
    }
    result = 1;
...
cleanup:
    ...
    if (!result) {
        ...
        lunarunlocker_log(L"materialized embedded product JAR: %ls (%lu bytes)",
                jar_path, size);
    }
    return result;
}
...
static jobject find_client_class_loader(JNIEnv *env) {
    ...
    if (error != JVMTI_ERROR_NONE || threads == NULL) {
        lunarunlocker_log(L"GetAllThreads failed: %d", error);
        return NULL;
    }
...
static int install_modular_payload_loader(
    ...
    if (runtime_loader_class == NULL) {
        lunarunlocker_log_pending_exception(env, L"resolve runtime ClassLoader type");
        return -1;
    }
    ...
    if (package_routes_field == NULL) {
        ...
        if (modular_loader_marker != NULL) {
            lunarunlocker_log(L"ModLauncher package routing field is unavailable");
            return -1;
        }
        if ((*env)->ExceptionCheck(env)) {
            (*env)->ExceptionClear(env);
    ...
    if (loader_class == NULL || url_loader_init == NULL
        ...
        lunarunlocker_log_pending_exception(env,
    ...
    payload_loader_name = (*env)->NewStringUTF(env,
            "gg.lunarunlocker.runtime.ForgePayloadClassLoader");
    ...
    if (build_package_routes == NULL || payload_loader == NULL
        ...
        lunarunlocker_log_pending_exception(env, L"create modular payload ClassLoader");
        return -1;
    }
    ...
    if (package_routes == NULL || (*env)->ExceptionCheck(env)) {
        lunarunlocker_log_pending_exception(env, L"build ModLauncher payload package routes");
        return -1;
    }
    (*env)->SetObjectField(env, *loader, package_routes_field, package_routes);
    if ((*env)->ExceptionCheck(env)) {
        lunarunlocker_log_pending_exception(env, L"install ModLauncher payload package routes");
        return -1;
    }

    
    target_event_name = (*env)->NewStringUTF(env,
            "gg.lunarunlocker.event.impl.EventRenderWorldPassExecutorDrain");
    ...
    *loader = payload_loader;
    lunarunlocker_log(L"installed payload package routes on ModLauncher ModuleClassLoader");
    return 1;
}
...
static int install_system_payload_search(
    ...
    if (g_jvmti == NULL) {
        lunarunlocker_log(L"JVMTI is unavailable for system ClassLoader search");
        return -1;
    }
    loader_class = (*env)->FindClass(env, "java/lang/ClassLoader");
    ...
    if (system_loader == NULL || (*env)->ExceptionCheck(env)) {
        lunarunlocker_log_pending_exception(env, L"ClassLoader.getSystemClassLoader");
        return -1;
    }
    ...
    if (error != JVMTI_ERROR_NONE) {
        lunarunlocker_log(L"AddToSystemClassLoaderSearch failed: %d", error);
        return -1;
    }
    ...
    lunarunlocker_log(L"appended product JAR to system ClassLoader search");
    return 1;
}
...
static int install_fabric_payload_search(
    ...
    if (launcher == NULL || (*env)->ExceptionCheck(env)) {
        lunarunlocker_log_pending_exception(env, L"FabricLauncherBase.getLauncher");
        return -1;
    }
    ...
    if (get_target_loader == NULL || add_to_class_path == NULL) {
        lunarunlocker_log_pending_exception(env, L"resolve FabricLauncher methods");
        return -1;
    }
    target_loader = (*env)->CallObjectMethod(
            env, launcher, get_target_loader);
    if (target_loader == NULL || (*env)->ExceptionCheck(env)) {
        lunarunlocker_log_pending_exception(env, L"FabricLauncher.getTargetClassLoader");
        return -1;
    }
    ...
    if (path == NULL || allowed_prefixes == NULL
            || (*env)->ExceptionCheck(env)) {
        lunarunlocker_log_pending_exception(env, L"create Fabric payload path");
        return -1;
    }
    (*env)->CallVoidMethod(env, launcher, add_to_class_path,
            path, allowed_prefixes);
    if ((*env)->ExceptionCheck(env)) {
        lunarunlocker_log_pending_exception(env, L"FabricLauncher.addToClassPath");
        return -1;
    }
    ...
    if (payload_event_loader == NULL || (*env)->ExceptionCheck(env)
        ...
        if ((*env)->ExceptionCheck(env)) {
            ...
            lunarunlocker_log(L"Fabric payload event used the wrong ClassLoader");
        }
        return -1;
    }
    ...
    *loader = retained_target_loader;
    lunarunlocker_log(L"appended product JAR to Fabric Knot ClassLoader");
    return 1;
}
...
static int add_jar_to_loader(
    ...
    if (url_loader_class == NULL || url_class == NULL) {
        lunarunlocker_log_pending_exception(env, L"resolve URLClassLoader classes");
        return 0;
    }
    ...
    if (file_init == NULL || to_uri == NULL || to_url == NULL) {
        lunarunlocker_log_pending_exception(env, L"resolve product JAR URL methods");
        return 0;
    }
    path = new_wide_string(env, jar_path);
    ...
    if (url == NULL || (*env)->ExceptionCheck(env)) {
        lunarunlocker_log_pending_exception(env, L"create product JAR URL");
        return 0;
    }
    ...
    if ((*env)->IsInstanceOf(env, *loader, url_loader_class)) {
        ...
        if (add_url == NULL) {
            lunarunlocker_log_pending_exception(env, L"resolve URLClassLoader.addURL");
            return 0;
        }
        (*env)->CallVoidMethod(env, *loader, add_url, url);
        if ((*env)->ExceptionCheck(env)) {
            lunarunlocker_log_pending_exception(env, L"URLClassLoader.addURL");
            return 0;
        }
        return 1;
    }
    ...
    if (delegated_loader_field != NULL) {
        ...
        if (delegated_loader != NULL
            ...
            lunarunlocker_log(L"appended product JAR to ModLauncher delegated ClassLoader");
            return 1;
        }
    ...
    if (url_loader_init == NULL || urls == NULL) {
        lunarunlocker_log_pending_exception(env, L"resolve child URLClassLoader constructor");
        return 0;
    }
    (*env)->SetObjectArrayElement(env, urls, 0, url);
    ...
    if (child_loader == NULL || (*env)->ExceptionCheck(env)) {
        lunarunlocker_log_pending_exception(env, L"create child product URLClassLoader");
        return 0;
    }
    (*env)->DeleteLocalRef(env, *loader);
    *loader = child_loader;
    lunarunlocker_log(L"created child URLClassLoader for non-URL Minecraft ClassLoader");
    return 1;
}
...
static jclass load_bridge_class(JNIEnv *env, jobject loader) {
    ...
    jmethodID load_class = loader_class == NULL ? NULL : (*env)->GetMethodID(
            env, loader_class, "loadClass", "(Ljava/lang/String;)Ljava/lang/Class;");
    jstring name = (*env)->NewStringUTF(env, "gg.lunarunlocker.runtime.NativeBridge");
    jclass result;
    ...
    if (result == NULL || (*env)->ExceptionCheck(env)) {
        lunarunlocker_log_pending_exception(env, L"load gg.lunarunlocker.runtime.NativeBridge");
        return NULL;
    }
    return result;
}
...
static int set_current_context_class_loader(JNIEnv *env, jobject loader) {
    ...
    if (thread_class == NULL) {
        lunarunlocker_log_pending_exception(env, L"resolve java.lang.Thread");
        return 0;
    }
    ...
    if (current_thread == NULL || set_context_loader == NULL) {
        lunarunlocker_log_pending_exception(env, L"resolve Thread context ClassLoader methods");
        return 0;
    }
    thread = (*env)->CallStaticObjectMethod(env, thread_class, current_thread);
    if (thread == NULL || (*env)->ExceptionCheck(env)) {
        lunarunlocker_log_pending_exception(env, L"Thread.currentThread");
        return 0;
    }
    (*env)->CallVoidMethod(env, thread, set_context_loader, loader);
    if ((*env)->ExceptionCheck(env)) {
        lunarunlocker_log_pending_exception(env, L"Thread.setContextClassLoader");
        return 0;
    }
    return 1;
}
...
static int pin_native_module(void) {
    ...
    if (!GetModuleHandleExW(GET_MODULE_HANDLE_EX_FLAG_FROM_ADDRESS
        ...
        lunarunlocker_log(L"GetModuleHandleExW(PIN) failed: %lu", GetLastError());
        return 0;
    }
    return 1;
}
...
static int call_bridge_start(JNIEnv *env, jclass bridge_class) {
    ...
    if (start == NULL) {
        lunarunlocker_log_pending_exception(env, L"resolve NativeBridge.start");
        return 0;
    }
    (*env)->CallStaticVoidMethod(env, bridge_class, start);
    if ((*env)->ExceptionCheck(env)) {
        lunarunlocker_log_pending_exception(env, L"NativeBridge.start");
        return 0;
    }
    return 1;
}
...
JNIEXPORT jint JNICALL JNI_OnLoad(JavaVM *vm, void *reserved) {
    ...
    bridge_class = (*env)->FindClass(env, "gg/lunarunlocker/runtime/NativeBridge");
    if (bridge_class == NULL
            || lunarunlocker_register_native_bridge(env, bridge_class) != JNI_OK) {
        lunarunlocker_log_pending_exception(env, L"JNI_OnLoad NativeBridge registration");
        return JNI_ERR;
    }
    lunarunlocker_log(L"JNI_OnLoad completed");
...
JNIEXPORT void JNICALL JNI_OnUnload(JavaVM *vm, void *reserved) {
    ...
    (void)reserved;
    if ((*vm)->GetEnv(vm, (void **)&env, JNI_VERSION_1_8) == JNI_OK) {
        lunarunlocker_release_native_bridge(env);
    }
